...
class LunarRegolithSimulationDiscrete(gym.Env):
    """
    ...
    def __init__(self,
        ...
        """
        super(LunarRegolithSimulationDiscrete, self).__init__()
    ...
    def step(self, action):
        ...
        """
        # Convert the discrete action to a continuous control signal
        control_signal = self.discrete_to_continuous(action)
        self.sim.data.ctrl[:] = control_signal
    ...
    def discrete_to_continuous(self, action):
        ...
        """
        # This method should be implemented based on your robot's control scheme
        # Here's a placeholder implementation
        if action == Action.STOP.value:
            return np.zeros(self.sim.model.nu)
        elif action == Action.MOVE_FORWARD.value:
            return np.array([1.0, 1.0])  # Assuming two wheel motors
        elif action == Action.MOVE_BACKWARD.value:
            return np.array([-1.0, -1.0])
        elif action == Action.TURN_LEFT.value:
            return np.array([-1.0, 1.0])
        elif action == Action.TURN_RIGHT.value:
            return np.array([1.0, -1.0])
        elif action == Action.DIG.value:
            return np.array([0.0, 0.0, 1.0])  # Assuming a third actuator for digging
        elif action == Action.DROP.value:
            return np.array([0.0, 0.0, -1.0])  # Reverse of digging
        else:
